...
def test_condition():
    ...
    # 关于可重入锁的概念见https://docs.python.org/2/library/threading.html#rlock-objects
    # When the underlying lock is an RLock, it is not released using its release() method, since this may not actually unlock
    # the lock when it was acquired multiple times recursively. Instead, an internal interface of the RLock class is used,
    # which really unlocks it even when it has been recursively acquired several times. Another internal interface is then used to
